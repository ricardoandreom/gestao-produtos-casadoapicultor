...
from docx_generator import gerar_documento_word
...
def exportar_excel(historico: list):
    ...
    cor_header = "1A1916"; cor_accent = "E8B84B"
    cor_linha_par = "F7F4ED"; cor_linha_impar = "FFFFFF"
    ...
    border_thin = Border(bottom=Side(style="thin", color="DDDDDD"), right=Side(style="thin", color="DDDDDD"))
    ...
    for row_idx, enc in enumerate(historico, start=5):
        ...
        valores = [f"{enc['id']:04d}", enc["data"], enc["hora"], enc["cliente"], enc["cliente_tipo"], produtos_texto, total_unidades]
        ...
        for col_idx, valor in enumerate(valores, start=1):
            ...
            cell.alignment = Alignment(horizontal="center" if col_idx in (1,3,5,7) else "left", vertical="center", wrap_text=True)
            if col_idx == 5:
                cell.font = Font(name="Arial", bold=True, size=10, color="C9943A" if valor == "AM" else "2E8B57")
            elif col_idx == 7: cell.font = bold_font
            else: cell.font = normal_font
    if historico:
        total_row = len(historico) + 5
        ws.merge_cells(f"A{total_row}:F{total_row}")
        ws[f"A{total_row}"].value = f"Total de encomendas: {len(historico)}"
        ws[f"A{total_row}"].font = Font(name="Arial", bold=True, size=10, color=cor_header)
        ws[f"A{total_row}"].fill = PatternFill("solid", start_color="FDF8EE")
        ws[f"A{total_row}"].alignment = Alignment(horizontal="right", vertical="center")
        cell_total = ws.cell(row=total_row, column=7, value=f"=SUM(G5:G{total_row - 1})")
    ...
    ws.freeze_panes = "A5"
    wb.save(str(DATA_FILE.parent / "historico_encomendas.xlsx"))
...
def guardar_produto(novo: dict):
    ...
    nova_linha = {"nome_produto": novo["nome_produto"], "ref_interna": novo["ref_interna"],
                  "categoria_produto": novo["categoria_produto"], "qt_palete_AM_FR": novo["qt_palete_AM_FR"],
...
async def criar_encomenda(encomenda: EncomendaRequest):
    ...
    nova_encomenda = {"id": novo_id, "data": data_atual, "hora": hora_atual,
                      "cliente": encomenda.cliente, "cliente_tipo": encomenda.cliente_tipo,
                      "produtos": [{"nome": p.nome, "quantidade": p.quantidade, "notas": p.notas} for p in encomenda.produtos],
                      "doc_path": ""}
    nome_ficheiro = f"encomenda_{novo_id:04d}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    caminho_doc = DOCS_DIR / nome_ficheiro
    try:
        gerar_documento_word(nova_encomenda, str(caminho_doc))
        nova_encomenda["doc_path"] = str(caminho_doc)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro ao gerar documento Word: {str(e)}")
...
async def listar_encomendas():
    return JSONResponse(content=carregar_historico())

@app.get("/api/encomendas/{encomenda_id}/download")
async def download_encomenda(encomenda_id: int):
    historico = carregar_historico()
    encomenda = next((e for e in historico if e["id"] == encomenda_id), None)
    if not encomenda:
        raise HTTPException(status_code=404, detail="Encomenda não encontrada.")
    if not encomenda["doc_path"] or not Path(encomenda["doc_path"]).exists():
        raise HTTPException(status_code=404, detail="Documento Word não encontrado.")
    return FileResponse(path=encomenda["doc_path"],
        media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        filename=Path(encomenda["doc_path"]).name)

@app.get("/api/exportar-excel")
async def download_excel():
    excel_path = DATA_FILE.parent / "historico_encomendas.xlsx"
    if not excel_path.exists():
        exportar_excel(carregar_historico())
    return FileResponse(path=str(excel_path),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        filename="historico_encomendas.xlsx")

# ── Rotas: API Mapeamento ──────────────────────────
